...
std::unique_ptr<Resampler> choose_resampler(const SMCOptions& smc_options) {
  if(smc_options.resampler == "multinomial") {
    return std::make_unique<Multinomial>();
  } else if(smc_options.resampler == "residual") {
    return std::make_unique<Residual>();
  } else if(smc_options.resampler == "stratified") {
    return std::make_unique<Stratified>();
  } else if(smc_options.resampler == "systematic") {
