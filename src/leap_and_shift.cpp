...
ivec leap_and_shift(const ivec& current_rho, const Prior& prior) {
    ivec proposal = current_rho;
    int u = randi(distr_param(0, prior.n_items - 1));
    
    ivec support = join_cols(
      regspace<ivec>(std::max(1, current_rho(u) - 1), +1, current_rho(u) - 1),
      regspace<ivec>(current_rho(u) + 1, +1, std::min(prior.n_items, current_rho(u) + 1)));
