...
uvec leap_and_shift(const uvec &current_rho, unsigned int cluster,
                    const Prior &prior) {
  ...
  uvec support = setdiff(regspace<uvec>(std::max(1, rho_u - 1),
                                        std::min(prior.n_items, rho_u + 1)),
                         uvec(rho_u));
...
bool Particle::rejuvenate(unsigned int T, const Options &options,
                          const Prior &prior, const std::unique_ptr<Data> &data,
  ...
  for (size_t cluster{}; cluster < prior.n_clusters; cluster++) {
    alpha_proposal(cluster) = R::rlnorm(log(parameters.alpha(cluster)),
                                        std::max(.001, alpha_sd(cluster)));
    rho_proposal.col(cluster) =
        leap_and_shift(parameters.rho.col(cluster), cluster, prior);
  }

  Particle proposal_particle(
      options, StaticParameters{alpha_proposal, rho_proposal, parameters.tau},
      pfun);

  double log_ratio{};
  vec additional_terms =
      prior.alpha_shape * (log(alpha_proposal) - log(parameters.alpha)) -
      prior.alpha_rate * (alpha_proposal - parameters.alpha);

  for (size_t t{}; t < T + 1; t++) {
    proposal_particle.run_particle_filter(
        t, prior, data, pfun, distfun, resampler, options.latent_rank_proposal);
  }
  ...
  Rcpp::NumericVector probs =
      Rcpp::exp(proposal_particle.log_normalized_particle_filter_weights);
  int proposed_particle_filter =
      Rcpp::sample(probs.size(), 1, false, probs, false)[0];

  bool accepted{};
  if (log_ratio > log(R::runif(0, 1))) {
    this->parameters =
        StaticParameters{alpha_proposal, rho_proposal, parameters.tau};
    this->conditioned_particle_filter = proposed_particle_filter;
    this->log_incremental_likelihood =
        proposal_particle.log_incremental_likelihood;
